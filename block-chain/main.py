...
def receive_transaction_thread():
    ...
    while True:
        ...
        transaction = Transaction(from_ip=message['from_ip'], to_ip=message['to_ip'], value=message['value'], signature=message['signature'], time_stamp=message['time_stamp'])
        ...
        if transaction.check_signature(public_key):
            ...
            if common.checkTransactions(transactions):
                ...
                if len(transactions) == TRANSACTION_POOL_SIZE:
                    ...
                    new_block = Block(transactions=transaction_db.read(), previousHash=blockchain.getMaxHeightBlock().hash, parent=blockchain.getMaxHeightBlock())
                    # Start mining
                    mining_thread_instance = threading.Thread(target=mining_thread, args=(new_block, blockchain.getMaxHeightBlock().hash))
...
def receive_block_thread():
    ...
    while True:
        ...
        for msg in message['transactions']:
            ...
            transactions.append(transaction)
        new_block = Block(transactions=transactions, previousHash=message['previousHash'], time_stamp=message['timestamp'], nonce=message['nonce'], parent=blockchain.getMaxHeightBlock())
